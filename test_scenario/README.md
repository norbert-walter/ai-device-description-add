...
<img src="https://raw.githubusercontent.com/norbert-walter/ai-device-description-add/refs/heads/main/pictures/Test_Scenario_Dashboard.png" alt="Living Room" width="800">

Pic.: Control interfaces
...
Both documents describe the same household and the same preferences. The choice between them depends on the AI model being used, not on the content of the preferences themselves.
...
The step chain enforces a pattern that is elementary for acting in a real physical world but is typically not trained into small AI models: read the current state of all devices, interpret the situation, combine the user's request with the current situation into a plan, only then act, and finally verify the resulting states. Small models are optimized for execution efficiency. Left to their own approach, they tend to skip directly from the user's request to an action, omitting the observation and planning phases entirely. This shortcut works in purely conversational tasks but fails in physical device control, where the current state of the world is always the necessary starting point and the outcome of every action must be confirmed by a fresh measurement.
...
**Safety and autonomy are device-defined — and dependent on the AI following the rules.** In conventional systems, safety rules are configured in the central hub and enforced mechanically. In ADD, the autonomy level and confirmation requirements are encoded in the device's own ADD document. They travel with the device and apply to any AI agent that reads the document. This is a structural advantage over central rule stores that can be silently overridden by a configuration change.

However, ADD does not yet provide a technical guarantee that the AI will actually follow these rules. The entire concept relies on the AI agent correctly reading, correctly interpreting, and consistently applying the rules encoded in the ADD document — across all devices, across the full session, and under the pressure of ambiguous or contradictory user requests. Depending on the AI model, this is sometimes not the case. Frontier models perform well under structured prompting but are not infallible. Small local models are significantly more prone to rule drift, shortcutting, and context loss.

All rules in ADD are designed to enable safe operation, provided the AI is capable of following them. Whether and under what conditions different AI models actually do this reliably is the central question this work is investigating. What prompting structure, what instruction design, what level of explicit step enforcement, and what form of rule restatement are needed to make an AI agent follow ADD rules with sufficient reliability for real-world use — that is what this research project sets out to find out. That is an exciting open question.
